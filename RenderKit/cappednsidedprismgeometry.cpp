...
CappedNSidedPrismGeometry::CappedNSidedPrismGeometry(double r, int s)
{
  _slices = s;
  _numberOfVertexes = 2 * 6 * _slices;
  _numberOfIndices = 2 * 6 * _slices;
  ...
  for(int i=0;i<_slices;i++)
  {
    ...
    float4 position1 = float4(float(r * cosTheta), 1.0, float(r * sinTheta), 0.0);
    float4 position2 = float4(float(r * cosTheta2), 1.0, float(r * sinTheta2), 0.0);
    float4 position3 = float4(float(0.0), 1.0, float(0.0), 0.0);

    _vertexes[index] = RKVertex(position1, float4(0.0,1.0,0.0,0.0), float2());
    _indices[index] = short(index);
    index++;

    _vertexes[index] = RKVertex(position3, float4(0.0,1.0,0.0,0.0), float2());
    _indices[index] = short(index);
    index++;

    _vertexes[index] = RKVertex(position2, float4(0.0,1.0,0.0,0.0), float2());
    _indices[index] = short(index);
    index++;
  }
  ...
  for(int i=0;i<_slices;i++)
  {
    ...
    float4 position1 = float4(float(r * cosTheta), -1.0, float(r * sinTheta), 0.0);
    float4 position2 = float4(float(0.0), -1.0, float(0.0), 0.0);
    float4 position3 = float4(float(r * cosTheta2), -1.0, float(r * sinTheta2), 0.0);
