...
QByteArray ZipWriter::xzCompress(QByteArray data)
{
  QByteArray arr;
  ...
  ret_xz = lzma_easy_encoder (&strm, 6 | LZMA_PRESET_EXTREME, check);
  if (ret_xz != LZMA_OK)
  {
    return QByteArray();
  }

  in_len = static_cast<size_t>(data.size());
  in_buf = reinterpret_cast<uint8_t*>(data.data());
  strm.next_in = in_buf;
  ...
  do
  {
    ...
    out_len = OUT_BUF_MAX - static_cast<int>(strm.avail_out);
...
QByteArray ZipReader::xzUncompress(QByteArray data)
{
  lzma_stream strm = LZMA_STREAM_INIT;
